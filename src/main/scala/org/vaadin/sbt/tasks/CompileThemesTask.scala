...
import org.vaadin.sbt.VaadinPlugin.autoImport.{ compileVaadinThemes, vaadinThemes, vaadinThemesDir }
...
object CompileThemesTask {
  ...
  val compileThemesTask: Def.Initialize[Task[Seq[File]]] = Def.task {

    val themesDir = vaadinThemesDir.value
    val themes = vaadinThemes.value
    val target = (Keys.target in compileVaadinThemes).value
    val dependencyCp = (dependencyClasspath in Compile).value
